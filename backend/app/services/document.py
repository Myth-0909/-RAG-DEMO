...
async def process_document_stream(
    ...
    Yields SSE events showing the model's thinking process:
      1. convert  - Convert document to Markdown
    ...
    try:
        ...
        yield _event("convert", "thinking", {
            "message": "正在将文档转换为 Markdown...",
        })

        from app.services.document_converter import convert_document_to_markdown
        ...
        if not raw_text.strip():
            doc.status = "failed"
            doc.error_message = "文档转换结果为空"
            db.commit()
            yield _event("convert", "error", {"message": "文档转换结果为空"})
            return

        # Fix split tables: docling may break tables across page boundaries
        from app.services.chunking import fix_split_tables
        raw_text = fix_split_tables(raw_text)

        # Persist the converted markdown so it can be viewed later
        doc.converted_text = raw_text
        db.commit()

        yield _event("convert", "done", {
            "chars": len(raw_text),
        ...
        heading_boundaries = chunking_plan.get("analysis", {}).get("heading_boundaries", [])
        ...
        def _refine_thinking(token: str):
            loop.call_soon_threadsafe(refine_queue.put_nowait, token)

        def _run_refine():
            try:
                from app.services.llm_analyzer import refine_chunks_stream
                return refine_chunks_stream(
                    chunks,
                    filename=doc.original_filename,
                    on_thinking=_refine_thinking,
                )
            except Exception as e:
                logger.error(f"LLM refinement error: {e}")
                return None
            finally:
                loop.call_soon_threadsafe(refine_queue.put_nowait, None)
